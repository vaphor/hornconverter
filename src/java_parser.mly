...
%token <Localizing.extent> ANDEQ XOREQ OREQ NULL_LITERAL LIST ITERATOR
...
list_type:
| LIST LT var_type GT {List_type(fst $3), $1}

iterator_type:
| ITERATOR {Iterator_type, $1}
...
| modifiers_opt var_type variable_declarators SEMICOLON 
    { List.rev_map
	(fun (n,i) -> let (vn,array_suffix) = n in
                      { var_modifiers   = fst $1;
                        var_type        = lift_array (fst $2) array_suffix;
                        var_name        = vn;
                        var_initializer = i } ) $3 }
...
/*true iff array variable*/
			 
variable_declarator_id:
| IDENTIFIER dims_opt  { ($1, fst $2) }

variable_initializer:
| expression            { Expr_init $1 }
| array_initializer     { Tuple_init (fst $1) }
...
| var_type variable_declarators
    { List.rev_map
        (fun (n,i) -> let (vn,array_suffix)=n in
          { var_modifiers   = [];
            var_type        = lift_array (fst $1) array_suffix;
            var_name        = vn;
            var_initializer = i },
          (snd vn) ) $2 }
| FINAL var_type variable_declarators
    { List.rev_map
        (fun (n,i) -> let (vn,array_suffix)=n in (*if b then array*)
          { var_modifiers   = [Final];
            var_type        =  lift_array (fst $2) array_suffix;
            var_name        = vn;
            var_initializer = i },
...
| array_index LBRACK expression RBRACK {($1)@[$3]}
...
unary_expression:
| MULT unary_expression {Unary(Iterator_access, $2), fromto $1 $2}
| preincrement_expression           { $1 }
| predecrement_expression           { $1 }
